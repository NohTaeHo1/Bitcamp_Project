    ...
    public GatewayFilter apply(Config config) {
        return ((exchange, chain) -> {
            ...
            return chain.filter(exchange).then(Mono.fromRunnable(()->{
                if (config.isPostLogger())      log.info("Global Filter End: response code -> {}, {}, {}", exchange.getResponse().getStatusCode(), exchange.getResponse().getHeaders().getAccessControlAllowOrigin(), exchange.getResponse().toString());
            }));
        });
    }
}
