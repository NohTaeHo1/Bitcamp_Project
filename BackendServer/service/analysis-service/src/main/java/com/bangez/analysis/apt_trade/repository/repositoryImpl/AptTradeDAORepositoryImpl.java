package com.bangez.analysis.apt_trade.repository.repositoryImpl;

import com.bangez.analysis.apt_trade.model.AptTrade;
import com.bangez.analysis.apt_trade.repository.AptTradeDAORepositoty;
...
public class AptTradeDAORepositoryImpl implements AptTradeDAORepositoty {
    ...
    public Mono<Map<String, Long>> plotGraphAvgCostByDate() {
        ...
        Criteria criteria = Criteria.where("contract_date")
                .gte(YearMonth.now().minusYears(1).format(formatter)+"01")
                .lte(YearMonth.now().format(formatter)+"01");
        ...
        return operations.find(query, AptTrade.class)
                .flatMap(i-> Mono.just(AptTrade.builder()
                                ...
                                .build()))
                .collect(Collectors.groupingBy(AptTrade::getContractDate,
                        Collectors.collectingAndThen(Collectors.averagingLong(AptTrade::getTradePrice), Double::longValue)));
    }
